...
class TrainImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_path, pid, camid, clothes_id, parsing_label, unclo_path, clo_path = self.dataset[index]
        img = read_image(img_path)
        # front = read_image(front_path)
        unclo = read_image(unclo_path)
        clo = read_image(clo_path)
        # head = read_image(head_path)
        if self.transform is not None:
            img = self.transform[0](img)
            # front = self.transform[1](front)
            unclo = self.transform[0](unclo)
            clo = self.transform[0](clo)
            # head = self.transform[1](head)
        return img, pid, camid, clothes_id, parsing_label, unclo, clo
