...
class PRCC(object):
    ...
    def _process_dir_train(self, dir_path):
        ...
        for pdir in pdirs:
            ...
            for img_dir in img_dirs:
                ...
                # front_path = img_path.replace('/train/', '/train_mask_front/')
                unclo_path = img_dir.replace('/train/', '/train_rgb_unclo/')
                clo_path = img_dir.replace('/train/', '/train_rgb_clo/')
                # head_path = img_path.replace('/train/', '/train_mask_head/')
                parsing_path = img_dir.replace('/train/', '/train_parsing/').replace('.jpg', '.npy')
                parsing_label = self.parsing(parsing_path)
                dataset.append((img_dir, label, camid, clothes_id, parsing_label, unclo_path, clo_path))
    ...
    def parsing(self, file_path):
        ...
        """
        label_parsing = np.load(file_path)
        img_arr = np.uint8(label_parsing)
        toImg = transforms.ToPILImage()
        resize = transforms.Resize((24, 12))
        img = toImg(img_arr)
        img = resize(img)
        label_parsing = np.array(img)
        # label_parsing = np.resize(label_parsing, (16, 8))
...
if __name__ == '__main__':
    p = PRCC('/media/cw/My Passport/dataset')